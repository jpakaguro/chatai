...
export const openaiAPIKeyHandlerAtom = atom(
...
const openAIPayloadAtom = atom<OpenAIStreamPayload>((get) => {
...
// Where we keep current chat ID - (Read Only) - if currentChatAtom changes, then chatIDAtom changes
...
// Abort Controller for cancelling OpenAI message Stream
// AbortController is a built-in Web API and is used to cancel the asynchronous operation.
...
export const addMessageAtom = atom(
  ...
  async (get, set, action: "generate" | "regenerate" = "generate") => {
    ...
    // we not only add the message to the state , but we also need to add it to Supabase
    ...
    try {
      ...
      const decoder = new TextDecoder();  // used to decode stream of binary data to text. utf-8 is default encoding
      let done = false;
